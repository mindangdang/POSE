...
async def optimize_query_with_llm(user_query: str):
    ...
    2. 중심어 동의어 확장: Anchor,Fit,Color는 유의어 2~3개로 확장하여 OR 연산으로 묶어라. (예: 청바지 -> 데님 팬츠 OR 데님 진 OR 청바지)
    3. 속성어 키워드 변환: Variable은 다양한 패션 용어(ex:스트릿, 빈티지, 가먼트 다잉 워싱, 파워숄더 등)로 확장하여 OR 연산으로 묶어라.
    ...
    출력:
    {{
    "final_query": "(\\"빈티지\\" OR \\"아카이브\\") AND (\\"브라운\\") AND (\\"가죽자켓\\" OR \\"라이더자켓\\" OR \\"레더자켓\\")"
    }}
