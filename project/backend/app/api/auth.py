...
# 환경 변수 설정 (.env 파일에 GOOGLE_CLIENT_ID와 JWT_SECRET 추가 필요)
...
@router.get("/auth/me")
async def get_me(current_user: dict = Depends(get_current_user), conn=Depends(get_db_connection)):
    user_id = current_user.get("sub")
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute("SELECT id, email, name, profile_image FROM users WHERE id = %s", (user_id,))
        user = await cur.fetchone()
        if not user:
